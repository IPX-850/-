...
from PIL import Image  # 新增Pillow库导入
                ...
                try:
                    ...
                    if img_div:
                        ...
                        if img_tag and (img_url := img_tag.get('src')):
                            ...
                            # 生成文件名（新增格式转换逻辑）
                            original_basename = os.path.basename(img_url.split('?')[0])
                            name_part = os.path.splitext(original_basename)[0]
                            sanitized_name = sanitize_filename(name_part)
                            filename = f"{idx:03d}_{sanitized_name}.png"  # 强制使用.png扩展名
                            ...
                            if not os.path.exists(filepath):
                                ...
                                logger.info(f'已转换为PNG: {filename}')
